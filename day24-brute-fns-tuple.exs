...
defmodule ALUValidator do
  def is_valid(instructions, serial) do
    IO.inspect(serial)
    is_valid_internal(instructions, serial, {_w = 0, _x = 0, _y = 0, _z = 0})
  end
  ...
  defp is_valid_internal([{:read, r} | instructions], [input | serial], cpu),
    do:
      is_valid_internal(
        instructions,
        serial,
        update_cpu(cpu, r, fn _ -> input end)
      )

  defp is_valid_internal([{:exec, r, op, get_operand} | instructions], serial, cpu),
    do:
      is_valid_internal(
        instructions,
        serial,
        update_cpu(cpu, r, fn current -> op.(current, get_operand.(cpu)) end)
      )
